...
function RpcClient(opts) {
  ...
  this.port = opts.port || 6334;
  this.user = opts.user || 'coinuser';
  this.pass = opts.pass || 'coinpassword';
...
function rpc(request, callback) {
  ...
  var errorMessage = 'DigiByte JSON-RPC: ';
  ...
  var req = this.protocol.request(options, function(res) {
    ...
    res.on('end', function() {
      ...
      if (res.statusCode === 500 && buf.toString('utf8') === 'Work queue depth exceeded') {
        var exceededError = new Error('DigiByte JSON-RPC: ' + buf.toString('utf8'));
